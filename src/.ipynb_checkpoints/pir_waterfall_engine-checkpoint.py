...
def irr_newton(cashflows: np.ndarray, guess: float = 0.10) -> float:
    """
    Numerically more robust Newton-IRR.
    ...
    for _ in range(200):
        # Keep r in a numerically stable range
        r = min(max(r, -0.95), 5.0)
        ...
        for t, cf in enumerate(cashflows):
            ...
            if t > 0:
                deriv_denom = base ** (t + 1)
                if not np.isfinite(deriv_denom) or deriv_denom == 0:
                    return float("nan")
                d -= t * cf / deriv_denom

        if not np.isfinite(npv) or not np.isfinite(d):
            return float("nan")
        ...
        # Dampen Newton step to avoid explosive updates
